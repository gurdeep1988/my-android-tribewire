...
/**
 * * This helper class download images from the Internet and binds those with
...
public class ImageDownloader1 {
	...
	private final int minimumHeight = 10;

	private static ImageDownloader1 imageDownloader = null;

	private ImageDownloader1() {

	}

	public static ImageDownloader1 getInstance() {
		if (imageDownloader == null) {

			imageDownloader = new ImageDownloader1();
		}
	 ...
	 * associated to the ImageView if an error occurs. * @param url The URL of
	...
	/**
	 * * Same as download but the image is always downloaded and the cache is
	 * not used. Kept private at the moment as its interest is not clear.
	 */
	...
	/**
	 * * Returns true if the current download has been canceled or if there was
	 * no download in progress on this image view. * Returns false if the
	 * download in progress deals with the same url. The download is not stopped
	 * in that case.
	 */
		...
		try {
			...
			if (entity != null) {
				...
				try {
					...
					Bitmap bitmap = CommonFunctions
							.decodeSampledBitmapFromResource1(inputStream, 200,
									200, entity, url);
			...
			getRequest.abort();
			Log.e(LOG_TAG, "I/O error while retrieving bitmap from " + url);
		} catch (IllegalStateException e) {
			getRequest.abort();
			Log.e(LOG_TAG, "Incorrect URL: " + url);
		} catch (Exception e) {
			getRequest.abort();
			Log.e(LOG_TAG, "Error while retrieving bitmap from " + url);
		} finally {
	...
	class BitmapDownloaderTask extends AsyncTask<String, Void, Bitmap> {
		...
		protected void onPostExecute(Bitmap bitmap) {
			...
			if (imageViewReference != null) {
				 ...
				 * it Or if we don't use any bitmap to task association
	...
	static class DownloadedDrawable extends BitmapDrawable {
		...
		public DownloadedDrawable(BitmapDownloaderTask bitmapDownloaderTask) {
			...
			super(context.getResources(),CommonFunctions.decodeBitmapResouece(context.getResources(), com.tribewire.app.R.drawable.image_placeholder, 200, 200));
	...
	/*
	 * in milliseconds Hard cache, with a fixed maximum capacity and a life
	 * duration
	 */
	...
	private Bitmap getBitmapFromCache(String url) {
		...
		synchronized (sHardBitmapCache) {
			...
			if (bitmap != null) {
				...
				/*
				 * Bitmap found in hard cache Move element to first position, so
				 * that it is removed last
				 */
	...
	public static void saveBitmap(String url, Bitmap bitmap) {
		try {
			...
			boolean isImageWritten = bitmap.compress(
			...
			File file1 = new File(CommonFunctions.getImageRL()
					+ requiredFileUrl);
			FileOutputStream fos = new FileOutputStream(file1);
			String uploadFilePath = file1.toString();
			// Log.e("", "The Compressed bitmap File path is " +
			// uploadFilePath);
			fos.write(bitmapImage);
			fos.close();
			// Bitmap bitmap3 = TribeWireUtil.getBitmap(context,
			// Uri.fromFile(file1));
			// Bitmap bitmap2 = TribeWireUtil.rotateBitmapZero(
			// Uri.fromFile(file1).toString(), bitmap3);
